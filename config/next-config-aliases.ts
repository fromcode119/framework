...
import type { ISourceAlias } from './interfaces/source-alias.interface';
import type { ISourceAliasInput } from './interfaces/source-alias-input.interface';
...
// (`packages/frontend/lib/bundling/frontend-runtime-vite-config.ts`) both read THESE lists and adapt them
...
export class NextConfigAliases {
   ...
   */
  static getSourceAliases(frontendDir: string): ISourceAlias[] {
    const src = (pkg: string) => path.resolve(frontendDir, '..', pkg, 'src');
    const alias = NextConfigAliases.sourceAlias;
   ...
   */
  static sourceAlias({ specifier, dir = '', entry = '', file = '', subpathsOnly = false }: ISourceAliasInput): ISourceAlias {
  ...
  static getServerOnlyStubFiles() {
    const stubs = NextConfigAliases.getBrowserStubsDir();
    const empty = NextConfigAliases.getEmptyModuleFile();
    return Object.fromEntries(NextConfigAliases.getServerOnlyStubPackages().map((pkg) => {
  ...
  static getNodeBuiltinFallbacks() {
    return {
      ...
      async_hooks: path.resolve(NextConfigAliases.getBrowserStubsDir(), 'async-hooks', 'index.ts'),
    };
  }

  /** The empty module a `false` builtin fallback resolves to in a bundler with no `fallback` option. */
  static getEmptyModuleFile() {
    return path.resolve(NextConfigAliases.getBrowserStubsDir(), 'empty-module.ts');
  }

  /** `getSourceAliases` in turbopack `resolveAlias` shape (paths relative to the project dir). */
  static toTurbopackResolveAlias(aliases: ISourceAlias[], frontendDir: string): Record<string, string> {
   ...
   */
  static toWebpackResolveAlias(aliases: ISourceAlias[]): Record<string, string> {
    const out: Record<string, string> = {};
    for (const alias of aliases) {
      if (alias.file) {
 ...
 */
NextConfigAliases.PRIVATE_PACKAGE_ALIASES = [
