...
| `integration:shipping_provider` | Declares the plugin as a shipping provider. Allows it to register rate-calculation handlers and order fulfilment callbacks via the shipping integration API. | `ctx.integrations.shipping.*` |
