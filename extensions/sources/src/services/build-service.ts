...
import type { IBuildResult } from '@plugin/src/services/interfaces/build-result.interface';
import type { IPackageBuiltEvent } from '@plugin/src/services/interfaces/package-built-event.interface';
...
export class BuildService {
  ...
  async checkForUpdates(): Promise<{ slug: string; type: string; hasUpdate: boolean; remoteSha: string | null }[]> {
    ...
    for (const entry of sources) {
      ...
      const remoteSha = await this.gitSync.getRemoteHeadSha(entry.gitUrl, entry.branch || 'main', entry.gitSecret || undefined);
