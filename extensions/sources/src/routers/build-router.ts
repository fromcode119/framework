import { AccessLevel, BaseRouter } from '@fromcode119/sdk/server';
 ...
 */
export class BuildRouter extends BaseRouter {
  constructor(
    private buildService: BuildService,
    private adminGuard: RequestHandler,
    /** The running plugin's real version, supplied by the registrar from `context.plugin`. */
  ...
  protected registerRoutes(): void {
    ...
    // Health check. The version is READ from the packed manifest — the same value the marketplace
    // stamps and the admin displays. It was previously the literal '1.0.0' while the plugin actually
    // shipped 0.1.x, so the probe reported a release that does not exist.
    this.get('/health', { access: AccessLevel.PUBLIC }, (req: Request, res: Response) => {
        res.json({
            status: 'healthy',
            slug: 'sources',
