...
/**
 * `next-build-codegen vite-config <sourceModule> <ClassName> <outFile>` — emit the Vite config entry for a theme or
 * plugin-UI build.
 *
 * Vite requires a default export from its config module; the authored configs are classes, so the
 * required export is generated at BUILD time — the same job next-build-codegen does for Next's route exports and
 * middleware.
 *
...
export class ViteConfigCommand extends NextorCommand {
  ...
  run(argv: string[]): number {
    ...
    if (!sourceModule || !className || !outFile) {
      console.error('usage: next-build-codegen vite-config <sourceModule> <ClassName> <outFile>');
      return 2;
    }
    ...
    console.log('[next-build-codegen] generated', path.basename(out));
    return 0;
  }
}
