...
/**
 * The single `next-build-codegen` entry point: `next-build-codegen <command> [args…]`.
 *
...
export class NextorCli {
  ...
  static async main(argv: string[]): Promise<number> {
    ...
    if (!command) {
      console.error(`[next-build-codegen] unknown command "${name}".`);
      return NextorCli.usage(2);
    }
    return await new command().run(rest);
  }

  /** Print `next-build-codegen --help` and return the given exit code, so callers can use it as a tail call. */
  private static usage(code: number): number {
    ...
    out(`Usage: next-build-codegen <command> [args…]\n\nCommands:\n${lines.join('\n')}`);
    return code;
  }
}
