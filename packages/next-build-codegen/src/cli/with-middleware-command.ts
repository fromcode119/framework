...
/**
 * `next-build-codegen with-middleware -- <command> [args…]` — run a Next command with the middleware glue present,
 * then REMOVE it.
 *
 ...
 * `typescript-multiple-inheritance build` uses when it rewrites sources for tsc and restores them. Interrupt the build and the
...
export class WithMiddlewareCommand extends NextorCommand {
  ...
  run(argv: string[]): Promise<number> | number {
    ...
    if (!command.length) {
      console.error('usage: next-build-codegen with-middleware -- <command> [args...]');
      return 2;
    }
    ...
    if (generated) {
      console.log(`[next-build-codegen] generated ${path.relative(packageDir, generated)} (removed when this command exits)`);
    }
    ...
    return new Promise<number>((resolve) => {
      ...
      child.on('error', (error: Error) => {
        cleanup();
        console.error(`[next-build-codegen] failed to run ${command.join(' ')}: ${error.message}`);
        resolve(1);
      });
    });
  }
  ...
  private static cleanup(generated: string | null | undefined, packageDir: string): void {
    ...
    try {
      ...
      console.log(`[next-build-codegen] removed ${path.relative(packageDir, generated)} — the tree keeps only the class`);
    } catch (error) {
      console.error(`[next-build-codegen] could not remove ${generated}: ${(error as Error).message}`);
    }
  }
}
