...
export class DependencyOverrideGuard {
  ...
  static run(): number {
    ...
    for (const failure of failures) {
      console.error(`[archor] dependency-overrides: ${failure}.`);
    }
    if (failures.length) {
      console.error('[archor] A security override changed. Re-audit it before changing the pin.');
      return 1;
    }

    console.log(`[archor] dependency-overrides: ${DependencyOverrideGuard.AUDITED.length} audited override(s) intact.`);
    return 0;
  }
