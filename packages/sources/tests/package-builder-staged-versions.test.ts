 ...
 * so `guestbook-` also matches `guestbook-extra-0.1.0`, which belongs to a different extension).
 */
describe('PackageBuilder — staged versions', () => {
  ...
  it('lists what is staged, newest first by NUMBER not by text', () => {
    stage('guestbook-0.1.9', 'guestbook-0.1.31', 'guestbook-0.1.20', 'guestbook-0.2.0');

    expect(builder.listStagedVersions(ExtensionScope.PLUGIN, 'guestbook'))
      .toEqual(['0.2.0', '0.1.31', '0.1.20', '0.1.9']);
  });

  it('does not claim another extension whose slug starts the same way', () => {
    // `guestbook-extra` is a different extension. A bare prefix match reads its package as version
    // "extra-0.1.0" of `guestbook` and offers it for install.
    stage('guestbook-0.1.31', 'guestbook-extra-0.1.0');

    expect(builder.listStagedVersions(ExtensionScope.PLUGIN, 'guestbook')).toEqual(['0.1.31']);
  });
  ...
  it('ignores files, counting only staged package directories', () => {
    stage('guestbook-0.1.31');
    fs.writeFileSync(path.join(root, 'plugins', 'packages', 'guestbook-0.9.9.zip'), 'not a package');
  ...
  it('answers an empty list when nothing has ever been built', () => {
    ...
    expect(builder.listStagedVersions(ExtensionScope.PLUGIN, 'guestbook')).toEqual([]);
  });
  ...
  it('keeps prerelease versions in the list and below their release', () => {
    stage('guestbook-1.0.0', 'guestbook-1.0.0-rc.1');

    expect(builder.listStagedVersions(ExtensionScope.PLUGIN, 'guestbook')).toEqual(['1.0.0', '1.0.0-rc.1']);
  });
});
