 ...
 */
describe('BuildService — installing what was just built', () => {
  ...
  beforeEach(() => {
    ...
    service.logger = { info: vi.fn(), warn: vi.fn(), error: vi.fn(), debug: vi.fn() };
    service.buildSourceService = { recordAutoUpdateFailure: vi.fn(async () => undefined) };
    service.resolvePackageArtifact = vi.fn(async () => artifact());
  });

  it('installs the staged DIRECTORY, never an archive', async () => {
    await service.installBuilt('fromcode', { type: BuildSourceType.THEME });
  ...
  it('leaves a running extension alone when "update if already installed" is off', async () => {
    ...
    await service.installBuilt('fromcode', { type: BuildSourceType.THEME, autoUpdate: false });
  ...
  it('replaces a running extension when that setting is on', async () => {
    ...
    await service.installBuilt('fromcode', { type: BuildSourceType.THEME, autoUpdate: true });
  ...
  it('reads the stored flag however the driver returned it', async () => {
    ...
    await service.installBuilt('fromcode', { type: BuildSourceType.THEME, autoUpdate: 't' });
  ...
  it('sends core to the archive installer, which is the one thing still packaged that way', async () => {
    ...
    await service.installBuilt('core', { type: BuildSourceType.CORE });
  ...
  it('records the failure against the source and does not throw, so the build still counts', async () => {
    ...
    await expect(service.installBuilt('fromcode', { type: BuildSourceType.THEME })).resolves.toBeUndefined();
    expect(service.buildSourceService.recordAutoUpdateFailure).toHaveBeenCalled();
  });

  it('says so rather than guessing when a build recorded no package', async () => {
    service.resolvePackageArtifact = vi.fn(async () => artifact({ stagedDir: null }));
