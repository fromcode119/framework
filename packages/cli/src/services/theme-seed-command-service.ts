 ...
 */
export class ThemeSeedCommandService {
  static async run(argv: string[]): Promise<void> {
    const frameworkRoot = ThemeSeedCommandService.resolveFrameworkRoot();
    const options = ThemeSeedCommandService.parseArgs(argv);

    if (options.help) {
      ThemeSeedCommandService.printHelp();
      process.exit(0);
    }

    const themeRoot = ThemeSeedCommandService.resolveThemeRoot(frameworkRoot, options);
    ...
    ThemeSeedCommandService.ensureFrameworkPackageLinks(themeRoot, frameworkRoot);

    const firstPass = ThemeSeedCommandService.runSeedPass(frameworkRoot, seedFile, options.databaseUrl);
    if (firstPass !== 0) {
    ...
    const restarted = ThemeSeedCommandService.restartApiContainer(options.containerName);
    if (!restarted) {
      process.exit(0);
    }

    process.exit(ThemeSeedCommandService.runSeedPass(frameworkRoot, seedFile, options.databaseUrl));
  }

  /** Walk up from this file until the framework workspace root (the dir holding `packages/`) is found. */
  static resolveFrameworkRoot(): string {
    let dir = __dirname;
    while (dir !== path.dirname(dir)) {
      if (fs.existsSync(path.join(dir, 'packages')) && fs.existsSync(path.join(dir, 'package.json'))) return dir;
  ...
  static runSeedPass(frameworkRoot: any, seedFile: any, databaseUrl: any): any {
    const env = {
      ...process.env,
    };
  ...
  static ensureFrameworkPackageLinks(themeRoot: any, frameworkRoot: any): any {
  ...
  static restartApiContainer(containerName: any): any {
    if (!containerName) {
      return false;
    }
