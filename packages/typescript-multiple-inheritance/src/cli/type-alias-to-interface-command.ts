...
/**
 * `typescript-multiple-inheritance type-alias-to-interface <dir> [--apply]` — rewrite module-level `type X = { … }` object-shape
...
export class TypeAliasToInterfaceCommand extends TyporCommand {
  ...
  run(argv: string[]): number {
    ...
    if (!target || target.startsWith('--')) {
      console.error('usage: typescript-multiple-inheritance type-alias-to-interface <dir> [--apply]');
      return 2;
    }
