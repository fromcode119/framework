...
/**
 * `typescript-multiple-inheritance build [tsc args…]` — run `tsc` over sources that use typescript-multiple-inheritance's extended syntax.
 *
 ...
 *
 *   typescript-multiple-inheritance build              # plain `tsc`
 *   typescript-multiple-inheritance build -b           # build mode
 *   typescript-multiple-inheritance build -p tsconfig.json
 */
export class BuildCommand extends TyporCommand {
  readonly summary = 'Run tsc with typescript-multiple-inheritance extended syntax understood [tsc args…].';
  ...
  run(argv: string[]): number {
    ...
    try {
      ...
      // semantics (`tsc`, `tsc -b`, `tsc -p x`); typescript-multiple-inheritance only supplies the transformed source underneath.
  ...
  /**
   * A POSITIONAL path (`typescript-multiple-inheritance build packages/core`) makes tsc treat it as a file list, which silently
