...
/**
 * `typescript-multiple-inheritance destructure-fold <path> [--apply]` — fold module-level destructuring
...
export class DestructureFoldCommand extends TyporCommand {
  ...
  run(argv: string[]): number {
    ...
    if (!rel) {
      console.error('usage: typescript-multiple-inheritance destructure-fold <path> [--apply]');
      return 1;
    }
