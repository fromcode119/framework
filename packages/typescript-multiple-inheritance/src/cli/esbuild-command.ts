...
import { TyporEsbuildPlugin } from '../tsmi-esbuild-plugin';
import { TyporCommand } from './tsmi-command';

/**
 * `typescript-multiple-inheritance esbuild <entry> [flags…]` — esbuild, with typescript-multiple-inheritance's extended syntax understood.
 *
...
export class EsbuildCommand extends TyporCommand {
  readonly summary = 'esbuild with typescript-multiple-inheritance extended syntax understood <entry> [flags…].';
  ...
  async run(argv: string[]): Promise<number> {
    ...
    for (const arg of argv) {
      ...
      switch (flag) {
        ...
        // reason next-build-codegen generates Next's route exports — so it arrives as a footer, not a top-level call.
        ...
        default:
          console.error(`[typescript-multiple-inheritance esbuild] unsupported flag --${flag}. Add it to EsbuildCommand rather than dropping it.`);
          return 2;
      }
    }

    if (!options.entryPoints.length) {
      console.error('[typescript-multiple-inheritance esbuild] no entry point given.');
      return 2;
    }

    try {
      await build(options);
      console.log(`[typescript-multiple-inheritance esbuild] built ${options.entryPoints.join(', ')} -> ${options.outfile ?? options.outdir}`);
      return 0;
    } catch {
