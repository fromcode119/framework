 ...
 *
 * typescript-multiple-inheritance is STANDALONE — this knows how to find a workspace root, not anything about this project.
 */
export class WorkspaceRoot {
  static find(from: string = process.cwd()): string {
    ...
    for (;;) {
      ...
      if (parent === dir) throw new Error('[typescript-multiple-inheritance] could not locate the workspace root (no packages/ dir found)');
      dir = parent;
    }
  }
}
