...
/**
 * `typescript-multiple-inheritance module-constant-fold <path> [--apply]` — move module-level `const`/`let` onto the class that
...
export class ModuleConstantFoldCommand extends TyporCommand {
  ...
  run(argv: string[]): number {
    ...
    if (!rel) {
      console.error('usage: typescript-multiple-inheritance module-constant-fold <path> [--apply]');
      return 1;
    }
