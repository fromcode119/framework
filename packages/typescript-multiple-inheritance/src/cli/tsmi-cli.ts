...
/**
 * The single `typescript-multiple-inheritance` entry point: `typescript-multiple-inheritance <command> [args…]`.
 *
...
export class TyporCli {
  ...
  static async main(argv: string[]): Promise<number> {
    ...
    if (!command) {
      console.error(`[typescript-multiple-inheritance] unknown command "${name}".`);
      return TyporCli.usage(2);
    }
    return await new command().run(rest);
  }

  /** Print `typescript-multiple-inheritance --help` and return the given exit code, so callers can use it as a tail call. */
  private static usage(code: number): number {
    ...
    out(`Usage: typescript-multiple-inheritance <command> [args…]\n\nCommands:\n${lines.join('\n')}`);
    return code;
  }
}
