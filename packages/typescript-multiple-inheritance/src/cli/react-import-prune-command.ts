...
/**
 * `typescript-multiple-inheritance react-import-prune <path> [--apply]` — remove `import React from 'react'` where the binding is
...
export class ReactImportPruneCommand extends TyporCommand {
  ...
  run(argv: string[]): number {
    ...
    if (!rel) {
      console.error('usage: typescript-multiple-inheritance react-import-prune <path> [--apply]');
      return 1;
    }
