...
/**
 * `typescript-multiple-inheritance interface-split <path> [--apply]` — split a multi-interface file into one file per interface
...
export class InterfaceSplitCommand extends TyporCommand {
  ...
  run(argv: string[]): number {
    ...
    if (!rel) {
      console.error('usage: typescript-multiple-inheritance interface-split <path> [--apply]');
      return 1;
    }
