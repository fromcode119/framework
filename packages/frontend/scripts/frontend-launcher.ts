...
/**
 * The `atlantis-frontend` binary: runs the Next public frontend from its own package directory.
 *
 * It exists because `next` must be started with THIS package as its cwd — installed globally, via
 * npx, or as a workspace dependency, the frontend's `.next` build and its config live here, not
...
export class FrontendLauncher {
  ...
  /** `FRONTEND_PORT`, else `PORT`, else the frontend's conventional port. */
  private static get port(): string {
    return process.env.FRONTEND_PORT || process.env.PORT || '3002';
  }
  ...
  private static resolveNextBinary(): string {
    const requireFrom = createRequire(path.join(FrontendLauncher.packageDirectory, 'package.json'));
    try {
  ...
  private static argumentsFor(nextBin: string): string[] {
    const port = FrontendLauncher.port;
  ...
  static main(): void {
    const child = spawn(process.execPath, FrontendLauncher.argumentsFor(FrontendLauncher.resolveNextBinary()), {
      ...
      stdio: 'inherit',
      env: { ...process.env, PORT: FrontendLauncher.port },
    });
