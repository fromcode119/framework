...
/**
 * Registers the account chrome the framework itself owns: identity, sign-in security and sessions.
 *
 * Nothing here may belong to a business domain. A panel that renders records a plugin owns is registered
 * by that plugin, through the account-section registry — the framework cannot know such a record's shape
 * or vocabulary, so a placeholder for one is a guess wearing the framework's name.
 */
