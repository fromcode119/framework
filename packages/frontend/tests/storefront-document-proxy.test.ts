...
describe('StorefrontDocumentProxy', () => {
  ...
  it('passes every request through while the rollout flag is off', async () => {
    vi.stubEnv(StorefrontDocumentProxy.FLAG, '');
    expect(StorefrontDocumentProxy.enabled()).toBe(false);
    expect((await StorefrontDocumentProxy.handle(request('/'))).headers.get('x-middleware-rewrite')).toBeNull();
  });
