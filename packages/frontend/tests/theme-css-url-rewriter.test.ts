...
const sheet = 'http://api.framework.local/api/v1/themes/aurora/ui/aurora-theme.css';
...
describe('ThemeCssUrlRewriter', () => {
  it('resolves relative url() references against the stylesheet URL, as a linked stylesheet would', () => {
    ...
    expect(ThemeCssUrlRewriter.rewrite(css, sheet)).toBe(
      "@font-face { src: url('http://api.framework.local/api/v1/themes/aurora/ui/fonts/a.woff2') format('woff2'); } "
      + '.x { background: url(http://api.framework.local/api/v1/themes/aurora/ui/images/b.png); } '
      + '.y { background: url("http://api.framework.local/api/v1/themes/aurora/shared/c.svg"); }',
    );
  });
...
describe('ThemeHeadLink href forms', async () => {
  ...
  it('resolves absolute, root-relative and bare (theme ui asset) hrefs', () => {
    ...
    expect(ThemeHeadLink.from({ rel: 'preconnect', href: 'https://fonts.gstatic.com' }, api, 'aurora')?.href).toBe('https://fonts.gstatic.com');
    expect(ThemeHeadLink.from({ rel: 'preload', href: '/api/v1/themes/aurora/ui/x.webp' }, api, 'aurora')?.href).toBe('http://api.test/api/v1/themes/aurora/ui/x.webp');
    expect(ThemeHeadLink.from({ rel: 'preload', href: 'fonts/a.woff2', as: 'font' }, api, 'aurora')?.href).toBe(ApiPathUtils.themeUiAssetUrl(api, 'aurora', 'fonts/a.woff2'));
