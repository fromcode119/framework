  ...
  useEffect(() => {
    async function initializeSystem() {
      try {
        ...
        try {
          const healthRes = await fetchWithTimeout(`${apiUrl}/api/health`, {
            cache: 'no-store',
            credentials: 'include'
          });
          if (healthRes.ok) {
        ...
        // 3. Load active plugins (best effort; should not block the app shell)
        try {
          const res = await fetchWithTimeout(`${apiUrl}/api/plugins/active`, {
            cache: 'no-store',
            credentials: 'include'
          });
          ...
          console.warn('[useSystemStatus] plugin bootstrap failed, continuing without blocking UI:', pluginError);
        }
