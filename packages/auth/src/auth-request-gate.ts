...
/**
 * What every incoming request has to get past: identify the caller, then decide if they may.
 *
 * FAIL-CLOSED by construction. A request with no token, an unreadable token, a token for another
 * tenant, or a permission checker that has not been wired yet all end the same way — refused. There
 * is no branch here that lets a request through because something was missing, which is the property
 * that makes `context.auth.guard()` safe for a plugin to call directly.
 *
 * A token failure is logged SELECTIVELY: a browser hitting a public page without a session is not an
 * incident, and logging it drowns the ones that are.
 *
 * Split out of `AuthManager` (382 lines), which owns tokens, hashing and the validators this asks.
 */
export class AuthRequestGate {
  constructor(
    private readonly logger: any,
    private readonly verifyToken: (token: string, expected?: { tenantId?: string }) => Promise<IUser>,
    private readonly applyTenantRoles: (user: any, tenantId: string) => Promise<any>,
    private readonly apiKeyValidator: () => IApiKeyValidator | null,
    private readonly permissionChecker: () => UserPermissionChecker | null,
  ) {}
