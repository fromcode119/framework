 ...
 *
 * Text analysis, not a parse, and the compromises are deliberate. A renderer often reads a key
 * through a delegate or a sibling component, so those are followed one level; an alias
 * (`const { title } = data`) counts as a read; and a fallback chain counts every key in it, because
 * any of them can be the one that renders.
 *
 * Split out of `BlockFieldConformanceGuard` (328 lines), which decides what a mismatch MEANS. Every
 * method here is static and pure — given the same source it gives the same answer — so the rules can
 * be checked one at a time instead of only through a whole guard run.
 */
export class BlockFieldSourceReader {
  /**
   ...
   */
  static writtenKeysByBlock(src: string, blockFile: string): Map<string, Set<string>> {
    ...
    const delegates = BlockFieldSourceReader.siblingSources(blockFile, src, 2);
   ...
   */
  static withDelegates(text: string, delegates: Map<string, string>): string {
    let combined = text;
   ...
   */
  static siblingSources(file: string, src: string, depth: number): Map<string, string> {
    ...
    const visit = (fromFile: string, fromSrc: string, left: number): void => {
      ...
      for (const match of fromSrc.matchAll(/import\s+(?:type\s+)?\{([^}]+)\}\s+from\s+['"]([^'"]+)['"]/g)) {
        ...
        for (const ext of ['.tsx', '.ts']) {
          ...
          const source = BlockFieldSourceReader.read(candidate);
  ...
  static readKeys(src: string): Set<string> {
   ...
   */
  static aliasGroups(src: string): Array<Set<string>> {
  ...
  static fallbackKeys(src: string): Set<string> {
   ...
   */
  static rendererIndex(abs: string): Map<string, string> {
    const index = new Map<string, string>();
    const suffixed: Array<[string, string]> = [];
    for (const file of BlockFieldSourceReader.walk(abs, /renderers?[\\/].*\.tsx$/)) {
      const key = BlockFieldSourceReader.normalize(path.basename(file, '.tsx'));
      index.set(key, file);
  ...
  static *walk(dir: string, match: RegExp): Generator<string> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) yield* BlockFieldSourceReader.walk(full, match);
