...
/**
 * An appearance may not reach into plugin or framework internals.
 *
 ...
 */
export class AppearanceBoundaryGuard {
  ...
  static run(): number {
    ...
    const inEngine = (p: any) => p === ENGINE_DIR || p.startsWith(ENGINE_DIR + path.sep);
    const findings: any[] = [];
