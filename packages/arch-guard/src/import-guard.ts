 ...
 *     TOOLCHAIN packages (arch-guard, reactor, next-build-codegen, typescript-multiple-inheritance): those build with raw `tsc`, not through
 *     `typescript-multiple-inheritance-build`, so `AliasEmitRewrite` never runs on them and an alias would survive into their
 *     `dist` where Node cannot resolve it. typescript-multiple-inheritance additionally cannot use one at all — resolving the
 *     alias is its OWN build output, so it would have to be built to be buildable.
 *
 * Lives in arch-guard because it guards a structural boundary, like the SDK and DB guards beside it.
 */
export class ImportGuard {
  ...
  /**
   * Marker of a next-build-codegen-GENERATED module. Generated entries are exempt from the alias STYLE rule (they
   ...
   */
  private static readonly GENERATED = 'GENERATED by @fromcode119/next-build-codegen';

  /**
   * Each typescript-multiple-inheritance-built package's PRIVATE alias for its own `src`, as `package -> prefix`.
   *
   * Unlike the Next apps' `@/`, nothing rewrites these at runtime — `typescript-multiple-inheritance-build` resolves them to
