 ...
 *
 *   arch-guard plugin-alias <slug>           # DRY RUN — report what would change
 *   arch-guard plugin-alias <slug> --apply   # write the files
 *   arch-guard plugin-alias --all [--apply]  # every plugin
 *
...
export class PluginAliasCommand extends ArchorCommand {
  ...
  run(argv: string[]): number {
    ...
    if (!all && !slug) {
      console.error('Usage: arch-guard plugin-alias <slug> [--apply]   |   arch-guard plugin-alias --all [--apply]');
      return 1;
    }
