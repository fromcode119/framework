...
import { ApiPathUtils, ApiScopeClient, PluginFrontendRuntimeUtils } from '@fromcode119/core/client';
  ...
  static useRuntimeBridgeInstall(args: {
    ...
    React.useEffect(() => {
      ContextRuntimeBridge.installRuntimeBridge({
        // The static half — SDK utilities, reactor surface, icons, the memoised lazy shells, React —
        // shared with the pre-boot install so both hand bundles the same references.
