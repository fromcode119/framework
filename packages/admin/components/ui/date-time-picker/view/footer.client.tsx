...
export class DateTimePickerFooter extends PureReactor {
  ...
  render(): ReactNode {
    const { theme, showTime, timezone, zonedParts, onTimeChange, onQuickAction, onClear, onClose } = this;

    return (
      <>
        {showTime && (
          <div className={`mt-6 pt-6 flex flex-col gap-5 ${theme === ThemeMode.DARK ? 'border-t border-white/5' : 'border-t border-slate-200/80'}`}>
            <div className="flex items-center gap-3.5">
               ...
               <div className="flex flex-col">
                  <span className={`text-[12px] font-semibold tracking-tight ${theme === ThemeMode.DARK ? 'text-slate-200' : 'text-slate-900'}`}>Time Precision</span>
            ...
            <div className="flex items-center gap-3 justify-center">
              <div className="flex flex-col gap-2 w-20">
                ...
                <input
                  ...
                  onChange={(e) => onTimeChange(TimePart.HOURS, e.target.value)}
              ...
              <span className={`text-2xl font-bold pt-6 ${theme === ThemeMode.DARK ? 'text-slate-400' : 'text-slate-300'}`}>:</span>
              <div className="flex flex-col gap-2 w-20">
                ...
                <input
                  ...
                  onChange={(e) => onTimeChange(TimePart.MINUTES, e.target.value)}
                  className={`w-full h-12 rounded-xl text-center font-bold text-[15px] tracking-tight transition-all duration-150 outline-none ${
                    theme === ThemeMode.DARK
                      ? 'bg-slate-800/60 text-white focus:bg-slate-700/60 focus:ring-2 focus:ring-indigo-400/50 ring-1 ring-white/5'
                      : 'bg-slate-50 text-slate-900 focus:bg-white focus:ring-2 focus:ring-indigo-500/50 shadow-inner ring-1 ring-black/5'
        ...
        <div className={`mt-6 pt-6 flex flex-col gap-3.5 ${theme === ThemeMode.DARK ? 'border-t border-white/5' : 'border-t border-slate-200/80'}`}>
