...
export const ArrayField = ({ field, value = [], onChange, theme, collectionSlug, pluginSettings }: ArrayFieldProps) => {
  ...
  const renderField = (f: any, item: any, index: number) => {
    ...
    if (f.type === 'relationship' || f.admin?.component === 'TagField' || f.admin?.component === 'Tags') {
       return (
        <TagField 
          {...fieldProps}
          ...
          hasMany={f.hasMany !== undefined ? f.hasMany : (f.admin?.component === 'TagField' || f.admin?.component === 'Tags')}
    ...
    return <Input {...fieldProps} type={f.type === 'number' ? 'number' : 'text'} className="font-bold" />;
  };
