...
export class ColorPicker extends AdminComponent {
  ...
  declare props: Pick<ColorPicker, 'value' | 'onChange' | 'disabled' | 'className' | 'size'>;
  ...
  @ref declare private containerRef: Ref<HTMLDivElement>;
  @ref declare private popoverRef: Ref<HTMLDivElement>;

  @bound
  private updatePosition(): void {
    if (this.containerRef.current) {
      const rect = this.containerRef.current.getBoundingClientRect();
      this.coords = { top: rect.bottom + 8, left: rect.left };
    }
  }
  ...
  private onOpenChange(isOpen: boolean): void {
    if (isOpen) this.addPositionListeners();
      ...
      {isOpen && (
        <RootFramework>
          ...
          >
            ...
            <div className={`mt-6 pt-6 border-t flex justify-end ${theme === ThemeMode.DARK ? 'border-white/5' : 'border-slate-100'}`}>
               <button
