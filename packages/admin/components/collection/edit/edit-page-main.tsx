...
export class EditPageMain extends PureReactor {
  ...
  render(): ReactNode {
    return (
      <>
        {this.standardMainFieldSections.map((section) => this.renderSection(section))}
        {this.fullWidthMainFieldSections.map((section) => this.renderSection(section, 'full-width-'))}
      </>
    );
  }
}
