 ...
 */
export class PluginTrendChart extends PureReactor {
  /** JSX props — the declared @prop fields, so call sites are type-checked without a <Props> generic. */
  declare props: Pick<PluginTrendChart, 'series' | 'xLabels' | 'height' | 'formatValue' | 'className'>;

  private static readonly DEFAULT_COLORS = ['#6366f1', '#10b981', '#f59e0b', '#f43f5e', '#0ea5e9'];

  @prop declare series: IPluginTrendSeries[];
  @prop declare xLabels: string[];
  @prop declare height?: number;
  @prop declare formatValue?: (value: number) => string;
  ...
  render(): React.ReactNode {
    ...
    const formatValue = this.formatValue ?? ((v: number) => String(v));
      ...
      <div className={className}>
        <div className="relative w-full" style={{ height: `${H}px` }}>
          <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="w-full h-full overflow-visible">
            <defs>
              {series.map((s, i) => {
                const color = s.color || PluginTrendChart.DEFAULT_COLORS[i % PluginTrendChart.DEFAULT_COLORS.length];
                return (
            ...
            {series.map((s: IPluginTrendSeries, i) => {
              const color = s.color || PluginTrendChart.DEFAULT_COLORS[i % PluginTrendChart.DEFAULT_COLORS.length];
