/**
 * Shared UI styling constants and utilities for consistent design across the framework.
 */

export const UI_COMMON = {
  // Common border radius for cards, inputs, buttons
  radius: 'rounded-lg',
  radiusInner: 'rounded-md',
  
  // Transitions
  transition: 'transition-all duration-200',
  
  // Shadows
  shadow: 'shadow-sm',
  shadowHover: 'shadow-md',
  shadowFocus: 'ring-4 ring-indigo-500/10',
};

// Base classes for any form field (Input, Select, Textarea)
export const UI_FIELD = {
  base: `w-full box-border appearance-none ${UI_COMMON.radius} outline-none border ${UI_COMMON.transition} font-semibold ${UI_COMMON.shadow}`,
  
  // Light mode
  light: 'bg-white border-slate-200 text-slate-900 placeholder:text-slate-400 hover:border-indigo-500 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10',
  
  // Dark mode
  dark: 'dark:bg-slate-900/60 dark:border-slate-800 dark:text-white dark:placeholder:text-slate-600 dark:hover:border-indigo-500/50 dark:focus:border-indigo-500 dark:focus:ring-0',
  
  // Sizes
  sizes: {
    sm: {
      layout: 'py-1.5 px-3 h-9 text-[12px] leading-none',
      container: 'min-h-[36px] py-1.5 px-3 leading-none'
    },
    md: {
      layout: 'py-2 px-3.5 h-10 text-[13px] leading-none',
      container: 'min-h-[40px] py-2 px-3.5 leading-none'
    },
    lg: {
      layout: 'py-2.5 px-4 h-11 text-sm leading-none',
      container: 'min-h-[44px] py-2.5 px-4 leading-none'
    }
  }
};

export const UI_TEXT = {
  LABEL: "text-[11px] font-semibold text-slate-500/80 mb-0.5 ml-0.5 tracking-wide",
  ERROR: "text-[11px] font-bold text-rose-500 mt-1 ml-0.5",
  SUBTEXT: "text-[11px] font-medium text-slate-400 mt-1 ml-0.5 leading-relaxed"
};

/**
 * Combines field classes into a single string
 */
export function getFieldClasses(size: keyof typeof UI_FIELD.sizes = 'md', extra: string = '', isContainer: boolean = false) {
  const sizeConfig = UI_FIELD.sizes[size];
  const sizeClasses = isContainer ? sizeConfig.container : sizeConfig.layout;
  
  // If no extras, return standard classes
  if (!extra) return `${UI_FIELD.base} ${UI_FIELD.light} ${UI_FIELD.dark} ${sizeClasses}`;

  // Smart merging: if 'extra' provides specific overrides, remove the defaults
  const hasHeight = extra.includes('h-');
  const hasPaddingY = extra.includes('py-') || extra.includes('pt-') || extra.includes('pb-');
  const hasPaddingX = extra.includes('px-');
  const hasPaddingLeft = extra.includes('pl-');
  const hasPaddingRight = extra.includes('pr-');
  const hasTextSize = extra.includes('text-');
  const hasRounded = extra.includes('rounded-');

  const finalSizeClasses = sizeClasses.split(' ').filter(cls => {
    if (hasHeight && cls.startsWith('h-')) return false;
    if (hasPaddingY && (cls.startsWith('py-') || cls.startsWith('pt-') || cls.startsWith('pb-'))) return false;
    if (hasPaddingX && (cls.startsWith('px-') || cls.startsWith('pl-') || cls.startsWith('pr-'))) return false;
    if (hasPaddingLeft && cls.startsWith('pl-')) return false;
    if (hasPaddingRight && cls.startsWith('pr-')) return false;
    if (hasTextSize && cls.startsWith('text-')) return false;
    return true;
  }).join(' ');

  const baseClasses = hasRounded
    ? UI_FIELD.base.split(' ').filter((cls) => !cls.startsWith('rounded')).join(' ')
    : UI_FIELD.base;

  return `${baseClasses} ${UI_FIELD.light} ${UI_FIELD.dark} ${finalSizeClasses} ${extra}`.replace(/\s+/g, ' ').trim();
}
