    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <button 
                ...
                className={`text-xs font-bold uppercase tracking-widest transition-colors ${!currentFolderId ? 'text-indigo-500' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}
              >
                Media
              </button>
              {folderPath.map((folder, index) => (
                <React.Fragment key={folder.id}>
                  ...
                  <button 
                    ...
                    className={`text-xs font-bold uppercase tracking-widest transition-colors ${index === folderPath.length - 1 ? 'text-indigo-500' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}
                  >
                    {folder.name}
                  </button>
                </React.Fragment>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" size="icon" className="rounded-xl" onClick={() => setIsFolderPromptOpen(true)}>
            <FolderPlus size={20} />
          </Button>
          <Button 
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1 group w-full">
          ...
          <input 
            ...
            className={`w-full rounded-xl py-3 pl-12 pr-4 outline-none border transition-all ${theme === 'dark' ? 'bg-slate-900 border-slate-800 text-white focus:border-indigo-500/50' : 'bg-white border-slate-200 text-slate-900 focus:border-indigo-500 shadow-sm'}`} 
          />
        </div>
        <div className={`flex items-center border rounded-xl p-1 transition-all duration-300 shadow-sm ${
          theme === 'dark' 
        ...
        <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" : "space-y-2"}>
          {folders.map(folder => (
            <Card 
              ...
              className={`group cursor-pointer hover:border-indigo-500/50 transition-all relative ${viewMode === 'list' ? 'p-3 flex items-center gap-4' : 'p-6 flex flex-col items-center justify-center text-center'}`}
              onClick={() => setCurrentFolderId(folder.id)}
            >
