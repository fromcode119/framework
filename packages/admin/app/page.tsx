...
export default class AdminPage extends AdminComponent<Record<string, never>, AdminPageState> {
  ...
  private async fetchStats(): Promise<void> {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        if (this.mounted) this.setState({ stats: sorted });
      }
    } catch (err) {
  ...
  private async fetchPlugins(): Promise<void> {
    try {
      ...
      if (Array.isArray(data)) {
        if (this.mounted) this.setState({ activePluginsCount: data.length });
      }
    } catch (err) {
  ...
  private async fetchUpdate(): Promise<void> {
    try {
      ...
      if (data && data.hasUpdate) {
        if (this.mounted) this.setState({ updateAvailable: data });
      }
    } catch (err) {
              ...
              <div className="px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-6 rounded-[1.9rem] bg-white/80 dark:bg-slate-900/40">
                ...
                <div className="flex items-center gap-3">
                  <Button
                    ...
                    onClick={() => this.setState({ updateAvailable: null })}
                  >
                    Dismiss
                  </Button>
                  <Button
                    ...
                    onClick={() => this.router.push(AdminConstants.ROUTES.SETTINGS.UPDATES)}
                  >
                    View Update Details
                  </Button>
                </div>
              ...
              <Card noPadding className="overflow-hidden border-0 bg-white shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)] ring-1 ring-slate-100 dark:bg-transparent dark:shadow-none dark:ring-0">
                <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 dark:border-slate-800">
                   ...
                   <Button
                     variant="ghost"
                     size="sm"
                     onClick={() => this.router.push('/plugins')}
