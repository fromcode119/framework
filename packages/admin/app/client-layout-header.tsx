...
import type { ClientLayoutHeaderState } from './client-layout-header.interfaces';
...
export class ClientLayoutHeader extends AdminComponent<ClientLayoutHeaderProps, ClientLayoutHeaderState> {
  private mounted = false;
  private intervalId: number | null = null;

  state: ClientLayoutHeaderState = {
    apiStatus: 'loading',
    isMaintenance: false,
  };
  ...
  private async checkStatus(): Promise<void> {
    try {
      ...
      if (this.mounted) this.setState({ apiStatus: 'online', isMaintenance: data.maintenance === true });
    } catch {
      if (this.mounted) this.setState({ apiStatus: 'offline' });
    }
  }
  ...
  private get userMenuItems() {
    ...
    return [
      ...
      ...(user?.roles?.includes('admin')
        ? [{ label: 'System Settings', icon: <Settings size={16} />, onClick: () => this.router.push(AdminConstants.ROUTES.SETTINGS.ROOT) }]
        : []),
  ...
  render(): React.ReactElement {
    ...
    const { apiStatus, isMaintenance } = this.state;
