...
import type { NewPermissionFormData, NewPermissionPageState } from './new-permission-page.interfaces';
...
export default class NewPermissionPage extends AdminComponent<Record<string, never>, NewPermissionPageState> {
  state: NewPermissionPageState = {
    loading: false,
    formData: {
      name: '',
      description: '',
  ...
  private patchForm(patch: Partial<NewPermissionFormData>): void {
  ...
  private async handleSubmit(e: React.FormEvent): Promise<void> {
    ...
    try {
      await AdminApi.post(AdminConstants.ENDPOINTS.SYSTEM.PERMISSIONS, this.state.formData);
      this.router.push(AdminConstants.ROUTES.USERS.PERMISSIONS);
    } catch (err) {
              ...
              <Card title="Capability Definition">
                <div className="space-y-6">
                  <Input
                    ...
                    value={formData.name}
                    onChange={(e) => this.patchForm({ name: e.target.value.toLowerCase().replace(/[^a-z0-9:]/g, '-') })}
                    required
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Input
                      ...
                      onChange={(e) => this.patchForm({ group: e.target.value })}
                      className="h-11 rounded-xl font-semibold"
                    />
                      ...
                      <div className="flex gap-2">
                        {['Low', 'Medium', 'High', 'Critical'].map(lvl => (
                          <button
                            key={lvl}
                            type="button"
                            onClick={() => this.patchForm({ impact: lvl })}
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 space-y-4">
                     ...
                     <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <div className={`h-1.5 w-1.5 rounded-full ${impactColors[formData.impact]}`} />
