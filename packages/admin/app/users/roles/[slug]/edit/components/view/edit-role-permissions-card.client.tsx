...
export class EditRolePermissionsCard extends PureReactor {
  /** JSX props — the declared @prop fields, so call sites are type-checked without a <Props> generic. */
  declare props: Pick<EditRolePermissionsCard, 'theme' | 'permissions' | 'selected' | 'onToggle'>;
              ...
              <div
                ...
                className={`p-4 rounded-xl border cursor-pointer transition-all duration-300 flex items-center justify-between group ${
                  selected.includes(perm.name)
                    ? (theme === ThemeMode.DARK ? 'bg-indigo-500/10 border-indigo-500/40' : 'bg-indigo-50 border-indigo-200')
