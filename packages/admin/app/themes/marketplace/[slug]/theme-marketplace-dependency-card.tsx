...
export class ThemeMarketplaceDependencyCard extends React.Component<ThemeMarketplaceChangelogProps> {
  render(): React.ReactNode {
    ...
    return (
      <Card className={`border-0 p-4 rounded-xl ${adminTheme === 'dark' ? 'bg-indigo-500/5' : 'bg-indigo-50/50'}`}>
        <h3 className={`text-[11px] font-bold uppercase tracking-widest mb-4 flex items-center gap-2 ${adminTheme === 'dark' ? 'text-indigo-400' : 'text-indigo-600'}`}>
          <FrameworkIcons.Puzzle size={16} />
          Dependency Guard
        </h3>
        <div className="space-y-3">
          {Object.entries(theme.dependencies).map(([slug, version]) => (
            <div key={slug} className={`flex items-center justify-between p-3 rounded-lg border ${adminTheme === 'dark' ? 'bg-slate-900/40 border-white/5' : 'bg-white border-slate-100'}`}>
              ...
              <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${adminTheme === 'dark' ? 'bg-slate-800' : 'bg-slate-50'}`}>
