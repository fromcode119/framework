    ...
    return (
      <>
        <div className="space-y-3">
          ...
          <p className={`text-base font-medium leading-relaxed ${adminTheme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
            {theme.description}
          </p>
        </div>
        ...
        {theme.changelog && theme.changelog.length > 0 && (
          <div className="space-y-4 mt-6">
            <div className="flex items-center gap-3">
              <div className={`h-6 w-1.5 rounded-full ${adminTheme === 'dark' ? 'bg-indigo-500/40' : 'bg-indigo-600'}`}></div>
            ...
            <div className="space-y-4">
              {theme.changelog.map((log, idx) => (
                <div key={idx} className="space-y-3">
                  ...
                  <div className={`rounded-xl border-0 overflow-hidden ${adminTheme === 'dark' ? 'bg-slate-900/40 ring-1 ring-white/5' : 'bg-white shadow-sm ring-1 ring-slate-100'}`}>
                    <ul className={`divide-y ${adminTheme === 'dark' ? 'divide-slate-800/50' : 'divide-slate-50'}`}>
                      {log.changes.map((change, cIdx) => (
                        <li key={cIdx} className={`p-4 flex items-start gap-4 transition-all duration-300 group ${adminTheme === 'dark' ? 'hover:bg-slate-800/30' : 'hover:bg-indigo-50/30'}`}>
