...
import type { ThemesMarketplacePageState } from './themes-marketplace-page.interfaces';

export default class ThemesMarketplacePage extends AdminComponent<Record<string, never>, ThemesMarketplacePageState> {
  private mounted = false;

  state: ThemesMarketplacePageState = {
    themes: [],
    installedThemes: [],
    loading: true,
  };
  ...
  render(): React.ReactElement {
    ...
    const { themes, installedThemes, loading } = this.state;
        ...
        <div className="w-full">
          {themes.length === 0 ? (
            <div className="py-12 text-center rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20">
              ...
              <h3 className={`text-base font-bold mb-1 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Marketplace empty</h3>
              ...
              {themes.map(t => {
                ...
                return (
                  <Card
                    ...
                    className={`group flex flex-col border-0 relative transition-all duration-300 cursor-pointer overflow-hidden rounded-xl ${theme === 'dark' ? 'bg-slate-900/40 hover:bg-slate-900/60 ring-1 ring-white/5' : 'bg-white shadow-sm hover:shadow-md'}`}
                  >
                    <div className="p-4 space-y-4 flex-1">
                      <div className="flex items-start justify-between">
                        <div className={`h-9 w-9 rounded-lg flex items-center justify-center ${theme === 'dark' ? 'bg-slate-800 text-indigo-400 ring-1 ring-white/10' : 'bg-indigo-50 text-indigo-600 ring-1 ring-indigo-100'}`}>
                      ...
                      <div className="space-y-2">
                        <h3 className={`text-lg font-bold tracking-tight leading-tight ${theme === 'dark' ? 'text-white' : 'text-slate-900'} group-hover:text-indigo-600 transition-colors`}>
                          {t.name}
                        </h3>
                        <p className={`text-sm leading-relaxed font-medium line-clamp-3 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
                      ...
                      <div className="pt-2 mt-auto space-y-3">
                        {installed && hasUpdate && (
                          <div className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${theme === 'dark' ? 'bg-amber-500/10 border-amber-500/20' : 'bg-amber-50 border-amber-100 shadow-sm'}`}>
                        ...
                        <div className="flex items-center gap-3">
                          {installed && !hasUpdate ? (
                            <button
                              ...
                              className={`w-full h-9 rounded-lg font-bold uppercase tracking-widest text-[11px] transition-all flex items-center justify-center gap-2 ${theme === 'dark' ? 'bg-slate-800 text-slate-400 hover:text-white' : 'bg-white border text-slate-400 hover:bg-slate-50 hover:text-indigo-600 shadow-sm'}`}
                            >
