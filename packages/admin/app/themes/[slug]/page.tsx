...
export default class ThemeSettingsPage extends AdminComponent<ThemeSettingsPageProps, ThemeSettingsPageState> {
  ...
  async componentDidMount(): Promise<void> {
    ...
    this.setState({ routeSlug: params.slug, resolved: true, activeTab: nextTab }, () => void this.fetchTheme());
  }

  componentDidUpdate(): void {
    if (this.state.resolved && this.runtime.plugins?.refreshVersion !== this.prevRefreshVersion) {
      this.prevRefreshVersion = this.runtime.plugins?.refreshVersion;
      void this.fetchTheme();
    }
  }
  ...
  private async fetchTheme(): Promise<void> {
    const slug = this.state.routeSlug;
    try {
      ...
      if (found) {
        ...
        if (marketMatch && this.mounted) {
          this.setState({ marketplaceVersion: marketMatch.version });
        }
      } else {
  ...
  private async handleSaveConfig(): Promise<void> {
    const { themeDetail, routeSlug, dbConfig, tempVariables, tempLayouts, tempSettings } = this.state;
    if (!themeDetail) return;
    const notify = this.runtime.notify.notify;
    const triggerRefresh = this.runtime.plugins?.triggerRefresh;
  ...
  private openDeleteConfirm(): void {
    if (!this.state.themeDetail) return;
  ...
  private async handleDelete(): Promise<void> {
    ...
    const notify = this.runtime.notify.notify;
    const triggerRefresh = this.runtime.plugins?.triggerRefresh;
    this.setState({ isDeleting: true, isDeleteConfirmOpen: false });
    try {
  ...
  private openRunSeedsConfirm(): void {
    if (!this.state.themeDetail) return;
    this.setState({ isRunSeedsConfirmOpen: true });
  }

  private async handleRunSeeds(): Promise<void> {
    const { themeDetail } = this.state;
    if (!themeDetail) return;
    const notify = this.runtime.notify.notify;
    const triggerRefresh = this.runtime.plugins?.triggerRefresh;
    this.setState({ isRunSeedsConfirmOpen: false, isReseeding: true });
    try {
      ...
      this.setState({ isReseeding: false });
    }
  }

  private openResetThemeConfirm(): void {
    if (!this.state.themeDetail) return;
    this.setState({ isResetThemeConfirmOpen: true });
  }

  private async handleResetTheme(): Promise<void> {
    const { themeDetail } = this.state;
    if (!themeDetail) return;
    const notify = this.runtime.notify.notify;
    const triggerRefresh = this.runtime.plugins?.triggerRefresh;
    this.setState({ isResetThemeConfirmOpen: false, isResettingTheme: true });
    try {
      ...
      this.setState({ isResettingTheme: false });
    }
  }

  private handleVariableChange(key: string, value: string): void {
    this.setState((prev) => ({ tempVariables: { ...prev.tempVariables, [key]: value } }));
  }

  private handleLayoutChange(key: string, value: string): void {
  ...
  private handleTabChange(tabId: 'overview' | 'settings'): void {
  ...
  render(): React.ReactElement | null {
    ...
    const settings = this.runtime.plugins?.settings;
    const {
      themeDetail,
      marketplaceVersion,
      loading,
      ...
      isDeleting,
      isDeleteConfirmOpen,
      isRunSeedsConfirmOpen,
      isResetThemeConfirmOpen,
      tempVariables,
      tempLayouts,
      tempSettings,
    } = this.state;
    ...
    return (
      <div className="w-full space-y-8 animate-in fade-in duration-500">
                ...
                <Card className={`border-0 relative overflow-hidden p-8 transition-all duration-500 rounded-[2.5rem] ${adminTheme === 'dark' ? 'bg-slate-900/40' : 'bg-white shadow-xl shadow-slate-200/50'}`}>
                  ...
                  <div className={`mt-10 pt-8 border-t ${adminTheme === 'dark' ? 'border-slate-800/80' : 'border-slate-100'} flex items-center justify-between`}>
                    ...
                    {themeDetail.state !== 'active' && (
                        <button
                            onClick={() => void this.handleActivate()}
                                      ...
                                      <div key={key} className={`flex items-center justify-between p-7 rounded-[2.5rem] transition-all duration-500 border group ${
                                          ...
                                          <div className="flex-1 mr-8">
                                              ...
                                              {type === 'select' ? (
                                                  <Select
                                                      value={value || ''}
                                                      onChange={(nextValue) => this.handleVariableChange(key, String(nextValue || ''))}
                                                  ...
                                                  <div className="flex items-center gap-4">
                                                       <div className="flex-1 relative group/font">
                                                          <input
                                                              type="text"
                                                              value={value}
                                                              onChange={e => this.handleVariableChange(key, e.target.value)}
                                                  ...
                                                  <ColorPicker
                                                      value={value}
                                                      onChange={(nextValue) => this.handleVariableChange(key, nextValue)}
                                                      className="w-full"
                                                  />
                                              ) : type === 'image' ? (
                                                  <div className="flex items-center gap-4">
                                  ...
                                  <div key={layout.id} className={`flex flex-col p-6 rounded-[2rem] transition-all duration-500 border ${
                                      ...
                                      <div className="mt-auto">
                                          <Select
                                              ...
                                              onChange={(nextValue) => this.handleLayoutChange(layout.id, String(nextValue || ''))}
                                              options={[
                                 ...
                                 >
                                   ...
                                   {type === 'boolean' ? (
                                     <Switch
                                       ...
                                       onChange={(checked) => this.handleSettingChange(key, checked)}
                                     />
                                   ) : type === 'select' ? (
                                     <Select
                                       value={String(rawValue ?? '')}
                                       onChange={(nextValue) => this.handleSettingChange(key, String(nextValue ?? ''))}
                                       options={(schema?.options || []).map((opt) => ({
                                         value: String(opt.value),
                                         label: opt.label
                                       }))}
                                     ...
                                     <input
                                       ...
                                       onChange={(e) => {
                                         ...
                                         this.handleSettingChange(key, next === '' ? '' : Number(next));
                                       }}
            ...
            <div className="space-y-8">
              ...
              <Card className={`border-0 p-8 rounded-[2rem] ${adminTheme === 'dark' ? 'bg-slate-900/40' : 'bg-white shadow-xl shadow-slate-200/50'}`}>
                ...
                <div className="space-y-3">
                  <button
                    onClick={() => this.openRunSeedsConfirm()}
                  ...
                  <button
                    onClick={() => this.openResetThemeConfirm()}
                    disabled={isReseeding || isResettingTheme}
        ...
        <ConfirmDialog
          ...
          onClose={() => this.setState({ isRunSeedsConfirmOpen: false })}
          onConfirm={() => void this.handleRunSeeds()}
        ...
        <ConfirmDialog
          ...
          onClose={() => this.setState({ isResetThemeConfirmOpen: false })}
          onConfirm={() => void this.handleResetTheme()}
        ...
        <ConfirmDialog
          ...
          onClose={() => this.setState({ isDeleteConfirmOpen: false })}
          onConfirm={() => void this.handleDelete()}
          title="Delete Theme?"
          description={
            themeDetail?.state === 'active'
