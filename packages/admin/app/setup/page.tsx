...
import type { SetupPageState } from './setup-page.interfaces';

const { Zap, Mail, Lock, ArrowRight, Shield: ShieldCheck, UserPlus, Orbit } = FrameworkIcons;

const ICON_SIZE = 42;

export default class SetupPage extends AdminComponent<Record<string, never>, SetupPageState> {
  ...
  async componentDidMount(): Promise<void> {
    ...
    try {
      ...
      if (this.mounted) this.setState({ isChecking: false });
    }
  }
  ...
  private async handleSubmit(e: React.FormEvent): Promise<void> {
    ...
    try {
      ...
      this.setState({ error: err.message || 'Setup failed. Please try again.' });
    } finally {
  ...
  render(): React.ReactElement {
    const { isChecking, error, email, password, confirmPassword, fieldErrors, isLoading } = this.state;
      ...
      <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-slate-50 dark:bg-[#020617]">
        <div className="w-full max-w-md">
          ...
          <div className="p-6 sm:p-7 rounded-3xl border shadow-[0_20px_40px_-12px_rgba(0,0,0,0.14)] dark:shadow-black/50 animate-in fade-in slide-in-from-bottom-8 duration-1000 bg-white dark:bg-slate-900/50 border-slate-100 dark:border-slate-800">
            ...
            <form onSubmit={(e) => this.handleSubmit(e)} className="space-y-5" noValidate>
