          ...
          {loading ? (
            ...
            filtered.map(plugin => {
              ...
              return (
                <Card
                  key={plugin.slug}
                  onClick={() => this.router.push(AdminConstants.ROUTES.PLUGINS.MARKETPLACE_DETAIL(plugin.slug))}
                ...
                >
                      ...
                      <div className={`h-16 w-16 rounded-2xl flex items-center justify-center transition-all duration-700 group-hover:rotate-3 group-hover:scale-110 shadow-lg flex-shrink-0 ${theme === 'dark' ? 'bg-slate-800 text-indigo-400 ring-1 ring-white/10' : 'bg-indigo-50 text-indigo-600 ring-1 ring-indigo-100 shadow-indigo-100'}`}>
                        {plugin.iconUrl && !hasImageError ? (
                          <img
                            ...
                            onError={() => this.setState(prev => ({ imageErrors: { ...prev.imageErrors, [plugin.slug]: true } }))}
                          />
                        ) : (
                  ...
                  <div className="px-8 pb-8 space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      {installed && !hasUpdate ? (
                        <button
                          onClick={(e) => { e.stopPropagation(); this.router.push(AdminConstants.ROUTES.PLUGINS.DETAIL(plugin.slug)); }}
                        ...
                        <button
                          onClick={(e) => this.handleInstall(e, plugin.slug, plugin.version)}
                          disabled={!!installing}
