  ...
  render(): React.ReactNode {
    ...
    return (
      <div className="space-y-4">
         ...
         <div className={`rounded-xl border-0 overflow-hidden ${theme === 'dark' ? 'bg-slate-900/40 ring-1 ring-white/5' : 'bg-white shadow-sm ring-1 ring-slate-100'}`}>
            <ul className={`divide-y ${theme === 'dark' ? 'divide-slate-800/50' : 'divide-slate-50'}`}>
               {plugin.changelog.map((log, idx) => (
                 <li key={idx} className={`p-4 flex items-start gap-4 transition-all duration-300 group ${theme === 'dark' ? 'hover:bg-slate-800/30' : 'hover:bg-indigo-50/30'}`}>
