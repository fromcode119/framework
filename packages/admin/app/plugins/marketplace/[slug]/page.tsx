    ...
    if (error || !plugin) {
      return (
        <div className="p-8 text-center space-y-4">
          ...
          <button onClick={() => this.router.push(AdminConstants.ROUTES.PLUGINS.MARKETPLACE)} className="text-indigo-600 font-semibold hover:underline">
            Back to Marketplace
          </button>
        </div>
      );
                            ...
                            <div className="ml-4">
                              <Dropdown
                                ...
                                items={allVersions.map(v => ({
                                  ...
                                  onClick: () => this.setState({ selectedVersion: v.version }),
             ...
             <Card noPadding className={`sticky top-8 overflow-hidden ${theme === 'dark' ? 'bg-[#0f172a] ring-1 ring-white/5 border-0' : 'bg-white shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] border-slate-100'}`}>
                <div className="p-8 space-y-8">
                  <div className="space-y-4">
                    {!installedPlugin ? (
                      <button
                        onClick={() => this.handleInstall(plugin.slug)}
                        disabled={installing}
