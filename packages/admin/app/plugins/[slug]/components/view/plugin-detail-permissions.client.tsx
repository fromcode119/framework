        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {plugin.manifest.capabilities && plugin.manifest.capabilities.length > 0 ? (
            plugin.manifest.capabilities.map((cap) => {
              ...
              return (
                <div key={cap} className={`flex items-center gap-3 p-3 ${AdminClass.SURFACE} transition-colors border ${isUnapproved ? (theme === ThemeMode.DARK ? 'bg-amber-500/5 border-amber-500/20' : 'bg-amber-50 border-amber-200') : (theme === ThemeMode.DARK ? 'bg-slate-800/30 border-white/5' : 'bg-white border-slate-100 hover:border-indigo-500/20')}`}>
