...
export abstract class BaseMigration {
   ...
   *
   * Resolves the `@plugin/table` shorthand and hands the rest to the driver: the DDL is the driver's
   * to write, and a driver that cannot re-type a column in place does nothing. It is safe to run
   * repeatedly, safe on a table that was never broken, and able to finish a half-applied run — see
   * `PostgresTextIdPrimaryKeyRepair` for why that takes two statements.
   *
  ...
  protected async repairTextIdPrimaryKey(
    ...
    await db.repairTextIdPrimaryKey(TableResolver.resolve(tableName));
  }
