...
describe('TenantIsolationSql', () => {
  ...
  it('never emits a bare current_setting comparison', () => {
    const all = TenantIsolationSql.statementsFor('pages').join('\n');
  ...
  it('columnStatementsFor adds the column and its index and NEVER enables row-level security', () => {
    ...
    // The half adoption runs BEFORE there are tenants. Enforcing here would hide every row on a
    // deployment that has none yet — the documented reason the boot sweep refuses to isolate
    // anything until a tenant exists.
    expect(statements).not.toContain('ROW LEVEL SECURITY');
    expect(statements).not.toContain('CREATE POLICY');
  });

  it('statementsFor is exactly the column half followed by the enforcement half', () => {
    expect(TenantIsolationSql.statementsFor('pages')).toEqual([
      ...TenantIsolationSql.columnStatementsFor('pages'),
      ...TenantIsolationSql.enforcementStatementsFor('pages'),
    ]);
  });

  it('emits column, index, enable, force and policy for a table', () => {
    const stmts = TenantIsolationSql.statementsFor('pages');
    expect(stmts[0]).toBe(
    ...
    expect(stmts[5]).toBe(
      ...
      + `USING (${TenantIsolationSql.predicate()}) WITH CHECK (${TenantIsolationSql.predicate()})`,
    );
  });
  ...
  it('rejects a table name that is not a plain identifier', () => {
    expect(() => TenantIsolationSql.statementsFor('pages"; DROP TABLE users; --')).toThrow(/identifier/i);
  });

  it('adds the column NULLABLE — a NOT NULL default that evaluates to NULL cannot be added to a populated table', () => {
    expect(TenantIsolationSql.statementsFor('pages')[0]).not.toContain('NOT NULL');
  });

  it('offers a parameterised backfill rather than inventing an owner for pre-tenancy rows', () => {
    expect(TenantIsolationSql.backfillStatement('pages'))
  ...
  it('can count rows that predate tenancy and are invisible to every tenant', () => {
    expect(TenantIsolationSql.unassignedCountStatement('pages'))
      .toBe('SELECT count(*)::int AS unassigned FROM "pages" WHERE "tenant_id" IS NULL');
  });
});

describe('TenantIsolationSql — taking a tenant-less deployment back out of isolation', () => {
  it('drops every named policy, then FORCE, then RLS — in that order', () => {
    const statements = TenantIsolationSql.removalStatementsFor('media', ['media_tenant_select', 'media_tenant_delete']);
  ...
  it('never drops the tenant_id column — repairing an installation must not lose ownership', () => {
    const statements = TenantIsolationSql.removalStatementsFor('pages', ['pages_tenant_isolation']).join(' ');
  ...
  it('refuses an injected table or policy name', () => {
    expect(() => TenantIsolationSql.removalStatementsFor('pages"; DROP TABLE users; --', [])).toThrow();
    expect(() => TenantIsolationSql.removalStatementsFor('pages', ['p"; DROP TABLE users; --'])).toThrow();
  });
  ...
  it('finds bespoke policies too, not only the generic *_tenant_isolation name', () => {
    ...
    const statement = TenantIsolationSql.isolatedPoliciesStatement();
  ...
  it('rewrites a tenant-blind UNIQUE constraint as (cols…, tenant_id), keeping default NULL semantics', () => {
    expect(TenantIsolationSql.scopeUniqueConstraintStatement('fcp_zeta_pages', 'fcp_zeta_pages_slug_key', ['slug']))
    ...
    expect(TenantIsolationSql.scopeUniqueIndexStatements('fcp_eta_meta', 'idx_fcp_eta_meta_content_unique', ['content_type', 'content_id'])).toEqual([
  ...
  it('finds tenant-blind uniques by catalog, never by a hand-kept list, and skips what a FOREIGN KEY depends on', () => {
    const constraints = TenantIsolationSql.tenantBlindUniqueConstraintsStatement();
    ...
    expect(TenantIsolationSql.tenantBlindUniqueIndexesStatement()).toContain('NOT x.indisprimary');
  });

  it('refuses an injected constraint or column name in the rewrite', () => {
    expect(() => TenantIsolationSql.scopeUniqueConstraintStatement('pages', 'x"; DROP TABLE pages; --', ['slug'])).toThrow();
    expect(() => TenantIsolationSql.scopeUniqueIndexStatements('pages', 'idx', ['slug"; --'])).toThrow();
  });
});
