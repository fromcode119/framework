...
const heldMap = () =>
  new Map<string, any>([
    ['ecommerce', { manifest: { slug: 'ecommerce' }, state: 'inactive', healthStatus: 'warning', heldReason: 'capability_drift' }],
    ['mlm', { manifest: { slug: 'mlm' }, state: 'error', healthStatus: 'error', error: 'boom' }],
    ['cms', { manifest: { slug: 'cms' }, state: 'active', healthStatus: 'healthy' }],
  ]);
...
describe('LifecycleService.summarizeHeldPlugins', () => {
  ...
  it('collects held + errored plugins as DATA only (no markup or copy)', () => {
    ...
    expect(ecommerce).toMatchObject({ held: true, reason: 'capability_drift' });
    ...
    const noReason = LifecycleService.summarizeHeldPlugins(
      new Map<string, any>([['x', { manifest: { slug: 'x' }, state: 'inactive', healthStatus: 'warning' }]]),
    )!;
