...
describe('LifecycleService.register() — install/update hooks', () => {
  ...
  it('does not call onInstall on subsequent registrations', async () => {
    const { service } = buildService({ 'test-plugin': { state: PluginState.INACTIVE, version: '1.0.0' } });
    const onInstall = vi.fn().mockResolvedValue(undefined);
  ...
  it('rehydrates a persisted capability-drift hold across reboot (stays held + warning) and self-heals stale health', async () => {
    const { service, manager, registry } = buildService({
      'test-plugin': { state: PluginState.INACTIVE, approvedCapabilities: [], healthStatus: PluginRegistryHealth.HEALTHY, heldReason: PluginHeldReason.CAPABILITY_DRIFT, version: '1.0.0' },
    });

    await service.register(makePlugin());

    const loaded = manager.plugins.get('test-plugin');
    expect(loaded?.state).toBe(PluginState.INACTIVE);
    expect(loaded?.heldReason).toBe(PluginHeldReason.CAPABILITY_DRIFT);
    expect(loaded?.healthStatus).toBe(PluginRegistryHealth.WARNING);
    // saved health was 'healthy' (stale) → re-assert the hold so the DB axis matches
    expect(registry.markPluginHeld).toHaveBeenCalledWith('test-plugin', PluginHeldReason.CAPABILITY_DRIFT);
    // must NOT run the healthy-reset savePluginState that would clobber the hold
    expect(registry.savePluginState).not.toHaveBeenCalledWith('test-plugin', PluginState.INACTIVE, undefined, '1.0.0');
  });
});
...
describe('LifecycleService.disable() — middleware cleanup', () => {
  it('calls unregisterByPlugin when disabling', async () => {
    ...
    const loadedPlugin = { ...makePlugin(), instanceId: 'x', state: PluginState.ACTIVE, approvedCapabilities: [], healthStatus: PluginRegistryHealth.HEALTHY };
    ...
    expect(loadedPlugin.state).toBe(PluginState.INACTIVE);
  });

  it('does nothing when plugin is already inactive', async () => {
    const { service, manager } = buildService({});
    const loadedPlugin = { ...makePlugin(), instanceId: 'x', state: PluginState.INACTIVE, approvedCapabilities: [], healthStatus: PluginRegistryHealth.HEALTHY };
    manager.plugins.set('test-plugin', loadedPlugin);
...
describe('LifecycleService.enable() — required route reconciliation failures', () => {
  it('fails plugin enable when required default-page reconciliation throws', async () => {
    const { service, manager, registry } = buildService({ 'test-plugin': { state: PluginState.INACTIVE, version: '1.0.0' } });
    const loadedPlugin = {
      ...makePlugin(),
      instanceId: 'x',
      ...
      healthStatus: PluginRegistryHealth.HEALTHY,
  ...
  it('clears stale startup error state after a successful enable', async () => {
    const { service, manager, registry } = buildService({ 'test-plugin': { state: PluginState.ERROR, version: '1.0.0', healthStatus: PluginRegistryHealth.ERROR } });
    const loadedPlugin = {
      ...makePlugin(),
      instanceId: 'x',
      ...
      path: '/plugins/test-plugin',
      approvedCapabilities: [],
      healthStatus: PluginRegistryHealth.ERROR,
      onEnable: vi.fn().mockResolvedValue(undefined),
    };
    manager.plugins.set('test-plugin', loadedPlugin);
    vi.spyOn((service as any).activation, 'autoDiscoverCollections').mockResolvedValue(undefined);
    ...
    expect(loadedPlugin.healthStatus).toBe(PluginRegistryHealth.HEALTHY);
    expect(registry.savePluginState).toHaveBeenCalledWith('test-plugin', PluginState.ACTIVE, expect.any(Array), '1.0.0');
  });
});
...
describe('LifecycleService.delete() — uninstall hook and cleanup', () => {
  it('calls onUninstall before removing plugin from map', async () => {
    ...
    const loadedPlugin = { ...makePlugin({ onUninstall }), instanceId: 'x', state: PluginState.INACTIVE, approvedCapabilities: [], healthStatus: PluginRegistryHealth.HEALTHY };
    manager.plugins.set('test-plugin', loadedPlugin);
