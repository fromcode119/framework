...
describe('LayoutRuntimeBridgeService', () => {
  ...
  it('prefers the highest-priority theme replacement for a page target', () => {
    ...
    expect(resolved.status).toBe(LayoutResolutionStatus.RESOLVED);
    expect(resolved.source).toBe(LayoutResolutionSource.THEME_REPLACEMENT);
  ...
  it('reports an equal-priority theme replacement conflict deterministically', () => {
    ...
    expect(resolved.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: LayoutDiagnosticCode.THEME_REPLACEMENT_CONFLICT, severity: LayoutDiagnosticSeverity.ERROR }),
      ]),
    );
  ...
  it('rejects a required page disable when no theme replacement exists', () => {
    ...
    expect(resolved.status).toBe(LayoutResolutionStatus.RESOLVED);
    expect(resolved.source).toBe(LayoutResolutionSource.PLUGIN);
    expect(resolved.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: LayoutDiagnosticCode.REQUIRED_ROUTE_DISABLED, severity: LayoutDiagnosticSeverity.ERROR }),
      ]),
    );
  });
  ...
  it('cross-checks backend contracts against frontend defaults and theme winners', () => {
    ...
    expect(diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: LayoutDiagnosticCode.THEME_OVERRIDE_SELECTED, targetKey: CATALOG_INDEX_TARGET }),
        expect.objectContaining({ code: LayoutDiagnosticCode.BACKEND_CONTRACT_PRESENT_FRONTEND_LAYOUT_MISSING, targetKey: CATALOG_DETAIL_TARGET }),
        expect.objectContaining({ code: LayoutDiagnosticCode.FRONTEND_LAYOUT_PRESENT_BACKEND_CONTRACT_MISSING, targetKey: CATALOG_ORPHAN_TARGET }),
      ]),
    );
  });
  ...
  it('keeps policy page recipe keys aligned when contracts and frontend defaults use page targets', () => {
    ...
    expect(diagnostics).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: LayoutDiagnosticCode.BACKEND_CONTRACT_PRESENT_FRONTEND_LAYOUT_MISSING }),
        expect.objectContaining({ code: LayoutDiagnosticCode.FRONTEND_LAYOUT_PRESENT_BACKEND_CONTRACT_MISSING }),
      ]),
    );
