...
export class PluginGuestDeclarations {
  ...
  declare(namespace: string, method: string, args: unknown[]): Promise<unknown> {
    const dropped: string[] = [];
    const portable = PluginGuestRemote.portable(args, dropped, this.keep);
    if (dropped.length) console.warn(`[plugin] context.${namespace}.${method}: dropped ${dropped.join(', ')} (not transferable)`);
    return this.send({ kind: String(PluginGuestRegistrationKind.DECLARATION.value), steps: [{ name: namespace }, { name: method, args: portable }] });
  }
}
