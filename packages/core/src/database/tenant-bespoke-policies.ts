import { JournalPolicySpec, PlatformKeysVisiblePolicySpec, SharedReadPolicySpec, TenantPolicySpec, TenantSettingsPolicySpec, UnownedReadPolicySpec } from '@fromcode119/database';
...
export class TenantBespokePolicies {
   ...
   */
  static specs(): TenantPolicySpec[] {
    return [
      // A shared asset is READABLE by every tenant and writable by none but its owner.
      new SharedReadPolicySpec('media', 'shared'),
      // A tenant may read the handful of deployment truths it cannot own.
      new PlatformKeysVisiblePolicySpec('_system_meta', 'key', TenantBespokePolicies.PLATFORM_KEYS),
      // A plugin's configuration is never platform-level.
      new TenantSettingsPolicySpec('_system_plugin_settings'),
      new JournalPolicySpec('_system_audit_logs'),
      new JournalPolicySpec('_system_logs'),
      ...
      new JournalPolicySpec('_system_notifications'),
      ...
      new UnownedReadPolicySpec('_system_email_suppressions'),
    ];
  }
}
