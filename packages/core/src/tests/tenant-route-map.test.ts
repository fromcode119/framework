...
const workspace = TenantRecord.from({ id: 'nexora', slug: 'nexora', primary_host: 'nexora.test', host_aliases: '["api.nexora.test"]', state: 'suspended', kind: 'workspace', appearance: 'nexora' });
...
describe('TenantRouteMap', () => {
  ...
  it('sends a site\'s hosts to the frontend and a workspace\'s hosts to the admin', () => {
    ...
    expect(map.resolve('nexora.test')?.target).toBe(GatewayTarget.ADMIN);
    expect(map.resolve('nexora.test')?.tenantId).toBe('nexora');
  });

  it('sends any api. alias to the api, whatever the kind', () => {
    expect(map.resolve('api.acme.test')?.target).toBe(GatewayTarget.API);
    expect(map.resolve('api.nexora.test')?.target).toBe(GatewayTarget.API);
  });
  ...
  it('routes a suspended tenant (the app answers 503 with the reason) and nothing else', () => {
    expect(map.resolve('nexora.test')).toBeDefined();
  ...
  it('round-trips through JSON, which is how the gateway receives it', () => {
    ...
    expect(copy.resolve('api.nexora.test')?.target).toBe(GatewayTarget.API);
  });
});
