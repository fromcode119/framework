...
describe('PluginTenantStateService', () => {
  ...
  it('DISABLING DELETES NOTHING — the tenant keeps its data and gets it back on re-enable', async () => {
    ...
    const db = fakeDb([{ tenant_id: 't1', plugin_slug: 'seo', state: 'active' }]);
    await new PluginTenantStateService(db).disable('t1', 'seo');
  ...
  it('invalidates the gate cache on every write — this IS the no-restart mechanism', async () => {
    ...
    spy.mockClear();
    await new PluginTenantStateService(fakeDb([{ tenant_id: 't1', plugin_slug: 'seo', state: 'active' }]))
      .disable('t1', 'seo');
    expect(spy).toHaveBeenCalledWith('t1');
    spy.mockRestore();
  });
  ...
  it('refuses a missing tenant or slug instead of writing a half-addressed row', async () => {
    ...
    await expect(service.enable('', 'seo')).rejects.toThrow(/required/i);
  ...
  it('lists only ACTIVE rows', async () => {
    const db = fakeDb([
      { tenant_id: 't1', plugin_slug: 'seo', state: 'active' },
      { tenant_id: 't1', plugin_slug: 'forms', state: 'inactive' },
    ]);
    expect(await new PluginTenantStateService(db).listEnabled('t1')).toEqual(['seo']);
  });
});
