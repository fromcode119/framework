...
describe('TenantIdRemap', () => {
  it('leaves values alone for tables that were not remapped, and unknown ids alone within remapped ones', () => {
    ...
    expect(remap.resolve('fcp_cms_pages', 7)).toBe(7);
    remap.set('fcp_cms_pages', 7, 108);
    remap.markRemapped('fcp_cms_pages');
    expect(remap.resolve('fcp_cms_pages', 7)).toBe(108);
    expect(remap.resolve('fcp_cms_pages', 8)).toBe(8);
    expect(remap.resolve('fcp_cms_pages', null)).toBeNull();
  });
...
describe('TenantArchiveManifest.from', () => {
  ...
  it('round-trips through toJSON', () => {
    const manifest = new TenantArchiveManifest(1, '2026-09-05T00:00:00.000Z', '0.1.88', 'single-tenant',
      ...
      [{ slug: 'seo', version: '0.1.33' }], { slug: 'fromcode', version: '0.1.27', config: null },
      [{ name: 'fcp_cms_pages', rows: 44, columns: ['id', 'title'], hasSerialId: true }], 28, { count: 19, bytes: 1000 }, []);
    ...
    expect(again.tableNames).toEqual(['fcp_cms_pages']);
  });
});
