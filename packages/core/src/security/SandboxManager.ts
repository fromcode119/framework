...
export class SandboxManager {
  private logger = new Logger({ namespace: 'SandboxManager' });
  ...
  public async applyPluginContext(ivmContext: ivm.Context, ctx: PluginContext) {
    ...
    // 4. Cache Bridge
    if (ctx.cache) {
      await jail.set('_cache_get', new ivm.Reference(async (key: string) => await ctx.cache.get(key)));
      await jail.set('_cache_set', new ivm.Reference(async (key: string, val: any, ttl?: number) => await ctx.cache.set(key, val, ttl)));
      await jail.set('_cache_del', new ivm.Reference(async (key: string) => await ctx.cache.del(key)));
