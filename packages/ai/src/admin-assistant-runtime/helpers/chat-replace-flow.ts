...
export class ChatReplaceFlowHelpers {
  static async handle(
    ...
    if (deterministicActions.length > 0 && fileReplaceActions.length === deterministicActions.length && fileReplaceActions.length > 1 && !IntentClassifier.isExplicitFileModificationIntent(message)) {
      const q = sanitize([`I found ${fileReplaceActions.length} source-file matches for "${replaceInstruction.from}" -> "${replaceInstruction.to}".`, 'Do you want to update content-record values instead, or should I apply these file changes?'].join(' '), ContextLevel.ADVANCED);
