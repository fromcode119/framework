...
export class AssistantConstants {
  ...
  static readonly PROVIDER_OPTIONS: SelectOption[] = [
    SelectOption.from({ value: 'openai', label: 'OpenAI' }),
    SelectOption.from({ value: 'ollama', label: 'Ollama' }),
    SelectOption.from({ value: 'anthropic', label: 'Anthropic' }),
    SelectOption.from({ value: 'gemini', label: 'Gemini' }),
  ];
