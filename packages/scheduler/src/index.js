...
const sdk_1 = require("@fromcode/sdk");
const node_cron_1 = __importDefault(require("node-cron"));
const logger = new sdk_1.Logger({ namespace: 'scheduler' });
class SchedulerService {
     ...
     */
    async register(name, schedule, handler, options = {}) {
     ...
     * Start the scheduler
     */
    async start(pulseIntervalMs = 60000) {
    ...
    calculateNextRun(schedule) {
        ...
        if (!match)
            return new Date(now.getTime() + 5 * 60000); // Default 5m if invalid
