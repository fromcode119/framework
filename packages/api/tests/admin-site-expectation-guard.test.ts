...
describe('AdminSiteExpectationGuard — a page saves only into the site it was opened for', () => {
  it('refuses a write from a page opened for another site (the session was switched in another tab)', () => {
    const refusal = AdminSiteExpectationGuard.refusal(request('PUT', '@platform'), 'example-site');
    expect(refusal).toMatchObject({ error: 'site_changed', expected: '@platform', current: 'example-site' });
  ...
  it('lets the write through when the page and the session agree', () => {
    expect(AdminSiteExpectationGuard.refusal(request('PUT', 'example-site'), 'example-site')).toBeNull();
  ...
  it('never refuses a read, a request that names no site, or the site switch itself', () => {
    expect(AdminSiteExpectationGuard.refusal(request('GET', 'acme'), 'example-site')).toBeNull();
    expect(AdminSiteExpectationGuard.refusal(request('PUT', undefined), 'example-site')).toBeNull();
    const select = ApiPathUtils.versioned(SystemConstants.API_PATH.AUTH.TENANTS_SELECT);
    expect(AdminSiteExpectationGuard.refusal(request('POST', 'acme', select), 'example-site')).toBeNull();
  });
});
