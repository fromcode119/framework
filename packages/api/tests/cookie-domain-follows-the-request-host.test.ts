...
describe('ApiUrlUtils.getCookieDomain', () => {
  ...
  it('uses COOKIE_DOMAIN for a host under it', () => {
    vi.stubEnv('COOKIE_DOMAIN', '.platform.test');
    expect(ApiUrlUtils.getCookieDomain('shop.platform.test')).toBe('.platform.test');
    expect(ApiUrlUtils.getCookieDomain('platform.test')).toBe('.platform.test');
  });

  it('ignores COOKIE_DOMAIN for a host outside it', () => {
    vi.stubEnv('COOKIE_DOMAIN', '.platform.test');
    expect(ApiUrlUtils.getCookieDomain('www.acme-store.example')).toBe('.acme-store.example');
  });
  ...
  it('never returns a public suffix', () => {
    ...
    expect(ApiUrlUtils.getCookieDomain('demo-shop.trycloudflare.com'))
