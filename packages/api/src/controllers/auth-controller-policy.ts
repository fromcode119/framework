...
import { systemSessions, eq, and } from '@fromcode/database';
...
export class AuthControllerPolicy extends AuthControllerInfrastructure {
  ...
  protected async getSessionDurationMinutes(): Promise<number> {
    try {
      const row = await this.readMetaRow('auth_session_duration_minutes');
  ...
  protected async getPasswordPolicySettings(): Promise<PasswordPolicySettings> {
    return {
      minLength: await this.getSettingNumber('auth_password_min_length', 8, 8, 128),
      requireUppercase: await this.getSettingBoolean('auth_password_require_uppercase', true),
      requireLowercase: await this.getSettingBoolean('auth_password_require_lowercase', true),
      requireNumber: await this.getSettingBoolean('auth_password_require_number', true),
      requireSymbol: await this.getSettingBoolean('auth_password_require_symbol', false),
      historyCount: await this.getSettingNumber('auth_password_history', 5, 0, 20),
      breachCheck: await this.getSettingBoolean('auth_password_breach_check', false)
    };
  }
  ...
  protected async getLoginThrottleSettings(): Promise<LoginThrottleSettings> {
    return {
      threshold: await this.getSettingNumber('auth_lockout_threshold', 5, 1, 50),
      windowMinutes: await this.getSettingNumber('auth_lockout_window_minutes', 15, 1, 1440),
      lockoutMinutes: await this.getSettingNumber('auth_lockout_duration_minutes', 30, 1, 43200),
      captchaEnabled: await this.getSettingBoolean('auth_captcha_enabled', false),
      captchaThreshold: await this.getSettingNumber('auth_captcha_threshold', 3, 1, 50)
    };
  }
