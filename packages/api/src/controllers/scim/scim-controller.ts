...
export class ScimController {
  ...
  private notFound(res: any, id: unknown): void {
    res.status(404).type(SCIM_CONTENT_TYPE).json({ schemas: [ERROR_SCHEMA], status: '404', detail: `User ${id} not found` });
  }

  private badRequest(res: any, detail: string): void {
    res.status(400).type(SCIM_CONTENT_TYPE).json({ schemas: [ERROR_SCHEMA], status: '400', detail });
  }
}
