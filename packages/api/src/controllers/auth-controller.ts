...
export class AuthController {
  private db: any;
  private readonly defaultSessionDurationMinutes = 10080; // 7 days
  private readonly minSessionDurationMinutes = 15;
  private readonly maxSessionDurationMinutes = 43200; // 30 days
  ...
  private async getSessionDurationMinutes(): Promise<number> {
    try {
      const db = (this.manager as any).db;
      const row = await db.findOne('_system_meta', { key: 'auth_session_duration_minutes' });
  ...
  private getCookieOptions(req: Request, isLogout = false, maxAgeMs?: number) {
