...
export class CollectionMiddleware extends BaseMiddleware {
  ...
  private buildRelationshipSearchTargets(
    ...
    // Collections are registered under their PHYSICAL slug (e.g. `fcp_<plugin>_products`), but a
    ...
    for (const field of (collection?.fields || [])) {
      ...
      // their PHYSICAL slug (`fcp_<plugin>_products`). Resolve logical→registered the same way the
