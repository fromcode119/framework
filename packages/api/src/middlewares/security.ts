...
export function csrfMiddleware(req: Request, res: Response, next: NextFunction) {
    ...
    if (req.headers.authorization || req.headers['x-api-key'] || req.headers['x-framework-client']) {
        return next();
    }
    ...
    if (!headerToken || csrfCandidates.length === 0 || !isValid) {
        console.warn(`[CSRF] Validation failed. Header: ${headerToken ? 'present' : 'missing'}, Candidates: ${csrfCandidates.length}`);
