...
export class ScimTokenService {
  ...
  async current(): Promise<string> {
    ...
    const row = await this.db.findOne(SystemConstants.TABLE.META, { key: TOKEN_KEY }).catch(() => null);
  ...
  async rotate(): Promise<string> {
    ...
    const now = new Date();
    const existing = await this.db.findOne(SystemConstants.TABLE.META, { key: TOKEN_KEY }).catch(() => null);
    if (existing) await this.db.update(SystemConstants.TABLE.META, { key: TOKEN_KEY }, { value: token, updatedAt: now });
    else await this.db.insert(SystemConstants.TABLE.META, { key: TOKEN_KEY, value: token, updatedAt: now });
    return token;
  }
}
